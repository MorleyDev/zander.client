...
class GetWithExistingOutOfDateArtefactsAndSourcesInTheCacheAndInstalledArtefactsThatAreUpToDateTests extends SpecTest {
  override def cmakeTestCase(compiler : String, mode: String, cmakeBuildType: String, generator: String) = {
    describe("Given the project/compiler endpoint exists and the cache already contains the source but out-of-date artefacts") {
      describe("When up-to-date artefacts are installed and get is carried out for %s.%s".format(compiler, mode)) {
