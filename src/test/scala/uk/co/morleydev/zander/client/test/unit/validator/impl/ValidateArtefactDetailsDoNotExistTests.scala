...
import uk.co.morleydev.zander.client.validator.impl.ValidateArtefactDetailsDoNotExist

class ValidateArtefactDetailsDoNotExistTests extends UnitTest {

  describe("Given artefact details do not exist When validating they do not exist") {
    ...
    Mockito.when(check.apply(Matchers.any[Project], Matchers.any[BuildCompiler], Matchers.any[BuildMode]))
      .thenReturn(false)
    val validate = new ValidateArtefactDetailsDoNotExist(check)
  ...
  describe("Given no artefact details exist When validating ") {
