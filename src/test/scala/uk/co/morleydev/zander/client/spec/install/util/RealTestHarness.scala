...
import uk.co.morleydev.zander.client.util.{CreateMockProcess, CreateMockHttpServer, TemporaryDirectory, MockServerAndPort}
...
import uk.co.morleydev.zander.client.spec.TestConfigurationFile
...
class RealTestHarness(parent : TestHarnessSpec) extends MockitoSugar with AutoCloseable {
  ...
  def expectSuccessfulRequest(gitUrl : String) : RealTestHarness = {
    ...
    expectRequestToThenReplyWith(endpointUrl, 200, responseBody)
  }


  def invokeMain() : RealTestHarness = {

    val configuration = new Configuration("http://localhost:" + theServerPort(), programs, cache.file.getAbsolutePath)
  ...
  def thenACMakePreBuildWasInvoked(buildType : String) : RealTestHarness = {
  ...
  def thenAMakeInstallWasInvoked() : RealTestHarness = {
